...
from playlist.utils.sql_utils import check_database_connection, check_table_exists
...
def db_check() -> Response:
    ...
    Raises:
        404 error if the songs table is not found.
    ...
    try:
        ...
        app.logger.info("Song table exists.")
    except Exception as e:
        app.logger.error(f"Failed to find songs table: {e}")
        return make_response(jsonify({
            'status': 'error',
            'message': 'Songs table not found',
            'details': str(e)
        }), 404)

    return make_response(jsonify({
        'status': 'success',
        'message': 'Database and songs table are healthy'
    }), 200)
